...
use crate::model::config::ClassifierPooling;
...
impl RotaryEmbedding {
    fn new(dtype: DType, config: &Config, rope_theta: f64, dev: &Device) -> Result<Self> {
        let dim =
            config.modernbert_config.hidden_size / config.modernbert_config.num_attention_heads;
        ...
        let max_seq_len = config.modernbert_config.max_position_embeddings;
...
impl ModernBertAttention {
    fn load(vb: VarBuilder, config: &Config, rotary_emb: Arc<RotaryEmbedding>) -> Result<Self> {
        let num_attention_heads = config.modernbert_config.num_attention_heads;
        let attention_head_size =
            config.modernbert_config.hidden_size / config.modernbert_config.num_attention_heads;

        let qkv = linear_no_bias(
            config.modernbert_config.hidden_size,
            config.modernbert_config.hidden_size * 3,
            vb.pp("Wqkv"),
        )?;
...
impl ModernBertMLP {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let wi = linear_no_bias(
            config.modernbert_config.hidden_size,
            config.modernbert_config.intermediate_size * 2,
            vb.pp("Wi"),
        )?;
        let wo = linear_no_bias(
            config.modernbert_config.intermediate_size,
            config.modernbert_config.hidden_size,
            vb.pp("Wo"),
        )?;
        Ok(Self { wi, wo })
...
impl ModernBertLayer {
    fn load(
        ...
        let attn_norm = layer_norm_no_bias(
            config.modernbert_config.hidden_size,
            config.modernbert_config.layer_norm_eps,
            vb.pp("attn_norm"),
        )
        .ok();
...
impl ModernBertClassifier {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let classifier = linear(
            config.modernbert_config.hidden_size,
            config.classifier_config.id2label.len(),
            vb.pp("classifier"),
        )?;
        Ok(Self { classifier })
    }
...
impl ModernBert {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let word_embeddings = embedding(
            config.modernbert_config.vocab_size,
            config.modernbert_config.hidden_size,
        ...
        let global_rotary_emb = Arc::new(RotaryEmbedding::new(
            vb.dtype(),
            config,
            config.modernbert_config.global_rope_theta,
            vb.device(),
        )?);
        let local_rotary_emb = Arc::new(RotaryEmbedding::new(
            vb.dtype(),
            config,
            config.modernbert_config.local_rope_theta,
            vb.device(),
        )?);

        let mut layers = Vec::with_capacity(config.modernbert_config.num_hidden_layers);
        for layer_id in 0..config.modernbert_config.num_hidden_layers {
            let layer_uses_local_attention =
                layer_id % config.modernbert_config.global_attn_every_n_layers != 0;
        ...
        Ok(Self {
            ...
            layers,
            final_norm,
            local_attention_size: config.modernbert_config.local_attention,
        })
    }
...
impl ModernBertForSequenceClassification {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            head,
            classifier,
            classifier_pooling: config.classifier_config.classifier_pooling,
        })
    }
    ...
    pub fn forward(&self, xs: &Tensor, mask: &Tensor) -> Result<Tensor> {
        ...
        let last_hidden_state = match self.classifier_pooling {
            ...
            ClassifierPooling::UNKNOWN => output,
        };
        let xs = self
            .head
